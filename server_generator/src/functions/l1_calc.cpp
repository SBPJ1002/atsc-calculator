...
std::string to_binary(int value, int bits) { return to_binary(static_cast<uint32_t>(value), bits);}
...
void fill_L1D_time_information() {
    ...
    preamble.L1D_time_sec = static_cast<uint32_t>(seconds_total & 0xFFFFFFFF);

    uint64_t fractional_ns = nanoseconds_total % 1000000000ULL;

    preamble.L1D_time_msec = static_cast<uint16_t>(fractional_ns / 1000000ULL);
    preamble.L1D_time_usec = static_cast<uint16_t>((fractional_ns % 1000000ULL) / 1000ULL);
    preamble.L1D_time_nsec = static_cast<uint16_t>(fractional_ns % 1000ULL);
...
int calculate_L1D_size_bits() {
    ...
    for(int i = 0; i < preamble.L1B_num_subframes; i++) {
        ...
        if (i == 0) {
            shouldAddSbsNullCells = (preamble.sub_frames[0].L1B_first_sub_sbs_first ||
                                   preamble.sub_frames[0].L1B_first_sub_sbs_last);
        } else {
            shouldAddSbsNullCells = (preamble.sub_frames[i].L1D_sbs_first ||
                                   preamble.sub_frames[i].L1D_sbs_last);
        }
        ...
        for(int j = 0; j < preamble.sub_frames[i].plps.size(); j++) {
            totalBits += 6;
            totalBits += 1;
            totalBits += 2;
            totalBits += 24;
            ...
            if(preamble.L1D_num_rf > 0) {
                ...
                if(preamble.sub_frames[i].plps[j].L1D_plp_num_channel_bonded > 0) {
                    totalBits += 2;
                    totalBits += 3 * preamble.sub_frames[i].plps[j].L1D_plp_num_channel_bonded;
                }
            }

            bool isMimo = (i == 0 && preamble.sub_frames[0].L1B_first_sub_mimo == 1) ||
            ...
            if(preamble.sub_frames[i].plps[j].L1D_plp_layer == 0) {
                ...
                if(preamble.sub_frames[i].plps[j].L1D_plp_TI_mode == 1) {
                    ...
                    if(preamble.sub_frames[i].plps[j].L1D_plp_HTI_inter_subframe == 0) {
                        ...
                        totalBits += 12 * (preamble.sub_frames[i].plps[j].L1D_plp_HTI_num_fec_blocks + 1);
                    }
                    totalBits += 1;
                }
            } else {
    ...
    for(int i = 0; i < preamble.L1B_num_subframes; i++) {
        ...
        bool shouldProcessMimo = (i == 0 && preamble.sub_frames[0].L1B_first_sub_mimo == 1 && preamble.sub_frames[0].L1B_first_sub_mimo_mixed == 1) ||
...
{
    const uint16_t bsr = static_cast<uint16_t>(bootstrap_info.bsr_coefficient);
    preamble.L1B_time_offset = calc_L1B_time_offset(
        bsr,
        static_cast<uint16_t>(preamble.L1D_time_usec),
        static_cast<uint16_t>(preamble.L1D_time_nsec)
    );
}
...
uint8_t count_total_plps()
{
    ...
    for (size_t i = 0; i < preamble.sub_frames.size() && i < preamble.L1B_num_subframes; i++) {
        total_plps += preamble.sub_frames[i].plps.size();
    }
...
{
    preamble.L1B_L1_Detail_total_cells = calc_L1B_L1_Detail_total_cells(
        static_cast<uint32_t>(preamble.L1B_L1_Detail_size_bytes),
        fec_mode,
        static_cast<uint8_t>(preamble.L1B_L1_Detail_additional_parity_mode)
    );
}

static const int TABLE_7_3_CELLS[3][5][8] = {
    // 8K FFT
    {
        {5711, 6285, 5999, 6429, 6287, 6573, 6431, 6645},  // cred=0
        {5631, 6197, 5915, 6339, 6199, 6481, 6341, 6552},  // cred=1
        {5552, 6110, 5832, 6250, 6112, 6390, 6252, 6460},  // cred=2
        {5473, 6023, 5749, 6161, 6025, 6299, 6163, 6368},  // cred=3
        {5394, 5936, 5666, 6072, 5938, 6208, 6074, 6276},  // cred=4
    },
    // 16K FFT
    {
        {11423, 12573, 11999, 12861, 12575, 13149, 12863, 13293},  // cred=0
        {11263, 12397, 11831, 12681, 12399, 12965, 12683, 13107},  // cred=1
        {11106, 12224, 11666, 12504, 12226, 12784, 12506, 12924},  // cred=2
        {10947, 12049, 11499, 12325, 12051, 12601, 12327, 12739},  // cred=3
        {10789, 11875, 11333, 12147, 11877, 12419, 12149, 12555},  // cred=4
    },
    // 32K FFT (SP4_x not valid for 32K, using 0)
    {
        {22847, 25149, 0, 0, 25151, 26301, 25727, 26589},  // cred=0
        {22527, 24797, 0, 0, 24799, 25933, 25367, 26217},  // cred=1
        {22213, 24451, 0, 0, 24453, 25571, 25013, 25851},  // cred=2
        {21895, 24101, 0, 0, 24103, 25205, 24655, 25481},  // cred=3
        {21579, 23753, 0, 0, 23755, 24841, 24299, 25113},  // cred=4
    }
};

static const int TABLE_7_4_CELLS[3][5][8] = {
    // 8K FFT
    {
        {6575, 6717, 6647, 6753, 6719, 6789, 6755, 6807},  // cred=0
        {6483, 6623, 6554, 6660, 6625, 6694, 6661, 6714},  // cred=1
        {6392, 6530, 6462, 6565, 6532, 6600, 6567, 6619},  // cred=2
        {6301, 6437, 6370, 6473, 6439, 6506, 6474, 6524},  // cred=3
        {6210, 6344, 6278, 6378, 6346, 6412, 6380, 6429},  // cred=4
    },
    // 16K FFT
    {
        {13151, 13437, 13295, 13509, 13439, 13581, 13511, 13617},  // cred=0
        {12967, 13249, 13109, 13320, 13251, 13391, 13322, 13428},  // cred=1
        {12786, 13064, 12926, 13134, 13066, 13204, 13136, 13239},  // cred=2
        {12603, 12877, 12741, 12946, 12879, 13015, 12948, 13051},  // cred=3
        {12421, 12691, 12557, 12759, 12693, 12827, 12761, 12861},  // cred=4
    },
    // 32K FFT
    {
        {26303, 26877, 26591, 27021, 26879, 27165, 27023, 27237},  // cred=0
        {25935, 26501, 26219, 26643, 26503, 26785, 26645, 26856},  // cred=1
        {25573, 26131, 25853, 26271, 26133, 26411, 26273, 26481},  // cred=2
        {25207, 25757, 25483, 25895, 25759, 26033, 25897, 26102},  // cred=3
        {24843, 25385, 25115, 25521, 25387, 25657, 25523, 25725},  // cred=4
    }
};
...
static const int SBS_TOTAL_DATA_CELLS[3][5][16] = {
    // 8K FFT
    {
        // cred=0: SP3_2, SP3_4, SP4_2, SP4_4, SP6_2, SP6_4, SP8_2, SP8_4, SP12_2, SP12_4, SP16_2, SP16_4, SP24_2, SP24_4, SP32_2, SP32_4
    ...
    // 32K FFT (SP4_x not valid, using 0)
    {
...
// Table F.1.1: Cred_coeff = 0
static const int SBS_ACTIVE_CELLS_CRED0[3][5][16] = {
    // 8K FFT - [sp_boost][sp_pattern]
    {
        // SPB=0: SP3_2, SP3_4, SP4_2, SP4_4, SP6_2, SP6_4, SP8_2, SP8_4, SP12_2, SP12_4, SP16_2, SP16_4, SP24_2, SP24_4, SP32_2, SP32_4
        {4560, 4560, 5136, 5136, 5712, 5712, 6000, 6000, 6288, 6288, 6432, 6432, 6576, 6576, 6648, 6648},
        // SPB=1
...
static bool is_table_7_3_pattern(int sp_pattern) {
    return sp_pattern < 8;  // SP3_2(0) to SP8_4(7)
}

static int get_cells_per_symbol(int fft_size, int reduced_carriers, int sp_pattern) {
    ...
    if (is_table_7_3_pattern(sp_pattern)) {
        return TABLE_7_3_CELLS[fft_size][reduced_carriers][sp_pattern];
    } else {
        return TABLE_7_4_CELLS[fft_size][reduced_carriers][sp_pattern - 8];
    }
}
...
static int calculate_subframe_data_capacity(int fft_size, int reduced_carriers, int sp_pattern,
                                            int num_ofdm_symbols, bool sbs_first, bool sbs_last) {
    int cells_per_symbol = get_cells_per_symbol(fft_size, reduced_carriers, sp_pattern);
    int sbs_data_cells = get_sbs_total_data_cells(fft_size, reduced_carriers, sp_pattern);

    int n_data_symbols = num_ofdm_symbols;
    if (sbs_first) n_data_symbols--;
    if (sbs_last) n_data_symbols--;

    int n_sbs_symbols = (sbs_first ? 1 : 0) + (sbs_last ? 1 : 0);

    int capacity = (n_data_symbols * cells_per_symbol) + (n_sbs_symbols * sbs_data_cells);
...
int calculate_sbs_null_cells(int subframe_index) {
    ...
    if (subframe_index == 0) {
        fft_size = preamble.sub_frames[0].L1B_first_sub_fft_size;
        reduced_carriers = preamble.sub_frames[0].L1B_first_sub_reduced_carriers;
        sp_pattern = preamble.sub_frames[0].L1B_first_sub_scattered_pilot_pattern;
        sp_boost = preamble.sub_frames[0].L1B_first_sub_scattered_pilot_boost;
        sbs_first = preamble.sub_frames[0].L1B_first_sub_sbs_first != 0;
        sbs_last = preamble.sub_frames[0].L1B_first_sub_sbs_last != 0;
    } else {
        fft_size = preamble.sub_frames[subframe_index].L1D_fft_size;
        reduced_carriers = preamble.sub_frames[subframe_index].L1D_reduced_carriers;
        sp_pattern = preamble.sub_frames[subframe_index].L1D_scattered_pilot_pattern;
        sp_boost = preamble.sub_frames[subframe_index].L1D_scattered_pilot_boost;
        sbs_first = preamble.sub_frames[subframe_index].L1D_sbs_first != 0;
        sbs_last = preamble.sub_frames[subframe_index].L1D_sbs_last != 0;
    }
...
void compute_and_set_all_sbs_null_cells() {
    for (int i = 0; i < preamble.L1B_num_subframes; i++) {
        preamble.sub_frames[i].L1D_sbs_null_cells = calculate_sbs_null_cells(i);
    }
}
