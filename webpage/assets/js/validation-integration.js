...
const originalLoadConfigBackup = window.loadConfigBackup;
...
const originalImportConfigFile = window.importConfigFile;
...
const originalToggleL1dBsid = window.toggleL1dBsid;
...
function initializeValidationOnLoad() {
    setTimeout(() => {
        if (window.atscValidation) {
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeValidationOnLoad);
} else {
    initializeValidationOnLoad();
}
...
window.showValidationStatus = showValidationStatus;
...
function getGIValueFromName(giName) {
    const map = {
...
let _modCodTablesLoaded = false;

async function loadModCodTables() {
    if (_modCodTablesLoaded) return true;
    try {
        const resp = await fetch('api.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'getValidationTables' })
        });
        const data = await resp.json();

        ATSC_MODULATION_CODING_TABLES = data.modulationCodingTables;
        FEC_TYPE_MAP = data.fecTypeMap;
        MODULATION_MAP = data.modulationMap;
...
window.submitForm = function() {
    ...
    if (originalSubmitFormModCod) {
        originalSubmitFormModCod();
    }
};
...
class CodeRateVisualValidator {
    ...
    getValidCodeRatesForCombination(fecType, modOrder) {
        ...
        return ATSC_MODULATION_CODING_TABLES[ldpcSize]?.[modOrder] || [];
    }
...
const codeRateValidator = new CodeRateVisualValidator();

const originalGeneratePLPMenusAndFieldsCodeRate = window.generatePLPMenusAndFields;
window.generatePLPMenusAndFields = function(subframeIndex) {
...
const originalLoadConfigBackupCodeRate = window.loadConfigBackup;
...
window.showCodeRateInfo = function(plpIndex) {
    codeRateValidator.showCodeRateTooltip(plpIndex);
};
