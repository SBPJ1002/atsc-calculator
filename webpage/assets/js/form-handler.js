...
function restoreFormData(formElement, formData) {
...
function saveAllSubframesData() {
    ...
    document.querySelectorAll('[id^="subframe"]').forEach(subframe => {
        ...
        if (match) {
            ...
            if (form) {
                savedData.subframes[subframeIndex] = saveFormData(form);
            }
        }
    });
    ...
    document.querySelectorAll('[id^="plp-"]').forEach(plp => {
        ...
        if (match) {
            ...
            if (form) {
                ...
                savedData.plps[subframeIndex][plpIndex] = saveFormData(form);
            }
        }
    });
...
function restoreAllData(savedData) {
    ...
    Object.keys(savedData.subframes || {}).forEach(subframeIndex => {
        ...
        if (subframe) {
            const form = subframe.querySelector('form');
            if (form) {
                restoreFormData(form, savedData.subframes[subframeIndex]);
            }
        }
    });
    ...
    Object.keys(savedData.plps || {}).forEach(subframeIndex => {
        Object.keys(savedData.plps[subframeIndex] || {}).forEach(plpIndex => {
            ...
            if (plp) {
                const form = plp.querySelector('form');
                if (form) {
                    restoreFormData(form, savedData.plps[subframeIndex][plpIndex]);
                }
            }
        });
    });
